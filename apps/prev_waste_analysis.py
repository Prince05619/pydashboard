...
def dynamo(date1,date2,ob):
    ...
    FilterExpression=Attr('timestamp').between('{0}'.format(date1),'{0}'.format(date2)) & Attr('object').eq('{0}'.format(ob))
    )
...
opts =[{'label': i, 'value': i} for i in waste_category]
layout = html.Div([
    html.H3('Select the Waste Category:'),
        dcc.Dropdown(
        id='select-type',
        options=[{'label': i, 'value': i} for i in waste_category],
        value='Aluminium_cans',
        multi=False,
            style={"width": "70%",
